 ...
 *
 * USAGE
 *   bun run tests/proba/harness/report.ts                    Latest run, table format
 *   bun run tests/proba/harness/report.ts --run 5            Specific run ID
 *   bun run tests/proba/harness/report.ts --format markdown  Markdown table
 *   bun run tests/proba/harness/report.ts --format csv       CSV output
 *   bun run tests/proba/harness/report.ts --format json      JSON output
 *   bun run tests/proba/harness/report.ts --failures         Show only failures
 */
...
async function main() {
    ...
    if (!existsSync(options.dbPath)) {
        ...
        console.error('Run the harness first: bun run tests/proba/harness/runner.ts');
        process.exit(1);
    }

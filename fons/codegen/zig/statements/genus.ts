...
export function genGenusDeclaration(node: GenusDeclaration, g: ZigGenerator): string {
    ...
    if (node.fields.length > 0) {
        lines.push('');
        lines.push(genStructInit(node, g));
    }
    ...
    if (node.methods.length > 0) {
        ...
        for (const method of node.methods) {
            lines.push(genStructMethod(method, g));
        }
    }
 ...
 */
function genStructInit(node: GenusDeclaration, g: ZigGenerator): string {
    const lines: string[] = [];

    lines.push(`${g.ind()}pub fn init(overrides: anytype) Self {`);
    g.depth++;

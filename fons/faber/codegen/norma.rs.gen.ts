 ...
 * Generated: 2026-01-15T14:30:37.481Z
 */
...
export const norma: Record<string, NormaCollection> = {
  "json": {
    "methods": {
      "solve": {
        "template": "serde_json::to_string(&§).unwrap()",
        "params": ["val"]
      },
      "solvePulchre": {
        "template": "serde_json::to_string_pretty(&§).unwrap()",
        "params": ["val","indentum"]
      },
      "pange": {
        "template": "serde_json::from_str(&§).unwrap()",
        "params": ["textus"]
      },
      "estNihil": {
        "template": "§.is_null()",
      ...
      "utNumerus": {
        "template": "§.as_i64().unwrap_or(0)",
        "params": ["val"]
      },
      "utBivalens": {
        "template": "§.as_bool().unwrap_or(false)",
        "params": ["val"]
      },
      "cape": {
        "template": "§.get(§).cloned().unwrap_or(serde_json::Value::Null)",
        "params": ["val","key"]
      },
      "capeIndice": {
        "template": "§.get(§).cloned().unwrap_or(serde_json::Value::Null)",
  ...
  "tempus": {
    "methods": {
      "nunc": {
        "template": "std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_millis() as i64",
        "params": []
      },
      "nunc_nano": {
        "template": "std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos() as i64",
        "params": []
      },
      "nunc_secunda": {
        "template": "std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() as i64",
        "params": []
      },
      "dormi": {
        "template": "std::thread::sleep(std::time::Duration::from_millis(§ as u64))",
        "params": ["ms"]
      },
      "MILLISECUNDUM": {
        "template": "1_i64",
      ...
      "HORA": {
        "template": "3600000_i64",
        "params": []
      },
      "DIES": {
        "template": "86400000_i64",
        "params": []
      }
    }
    , "innatum": "std::time"
  ...
  "aleator": {
    "methods": {
      "fractus": {
        "template": "rand::random::<f64>()",
        "params": []
      },
      "inter": {
        "template": "rand::thread_rng().gen_range(§0..=§1)",
        "params": ["min","max"]
      },
      "octeti": {
        "template": "{ let mut bytes = vec![0u8; §]; rand::thread_rng().fill(&mut bytes[..]); bytes }",
        "params": ["n"]
      },
      "uuid": {
        "template": "uuid::Uuid::new_v4().to_string()",
        "params": []
      },
      "selige": {
        "template": "§0.choose(&mut rand::thread_rng()).unwrap().clone()",
        "params": ["lista"]
      },
      "miscita": {
        "template": "{ let mut v = §0.clone(); v.shuffle(&mut rand::thread_rng()); v }",
        "params": ["lista"]
      },
      "semen": {
        "template": "/* seed(§) - use rand::SeedableRng for reproducible results */",
        "params": ["n"]
      }
    }
    , "innatum": "rand"

 ...
 * Generated: 2026-01-15T14:30:37.482Z
 */
...
export const norma: Record<string, NormaCollection> = {
  "json": {
    "methods": {
      ...
      "utTextus": {
        "template": "(§.is_string() ? §.get<std::string>() : \\\"\\\")",
        "params": ["val"]
      },
      "utNumerus": {
        "template": "(§.is_number() ? §.get<int64_t>() : 0)",
        "params": ["val"]
      },
      "utBivalens": {
        "template": "(§.is_boolean() ? §.get<bool>() : false)",
        "params": ["val"]
      },
      "cape": {
        "template": "(§.contains(§) ? §[§] : nlohmann::json())",
        "params": ["val","key"]
      },
      "capeIndice": {
        "template": "(§ < §.size() ? §[§] : nlohmann::json())",
        "params": ["val","idx"]
      }
    }
    , "innatum": "nlohmann::json"
  },
  "tempus": {
    "methods": {
      "nunc": {
        "template": "std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count()",
        "params": []
      },
      "nunc_nano": {
        "template": "std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count()",
        "params": []
      },
      "nunc_secunda": {
        "template": "std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count()",
        "params": []
      },
      "dormi": {
        "template": "std::this_thread::sleep_for(std::chrono::milliseconds(§))",
        "params": ["ms"]
      },
      "MILLISECUNDUM": {
        "template": "1",
  ...
  "aleator": {
    "methods": {
      "fractus": {
        "template": "[&]{ std::random_device rd; std::mt19937 gen(rd()); std::uniform_real_distribution<> dis(0.0, 1.0); return dis(gen); }()",
        "params": []
      },
      "inter": {
        "template": "[&]{ std::random_device rd; std::mt19937 gen(rd()); std::uniform_int_distribution<> dis(§0, §1); return dis(gen); }()",
        "params": ["min","max"]
      },
      "octeti": {
        "template": "[&]{ std::vector<uint8_t> bytes(§); std::random_device rd; for (auto& b : bytes) b = static_cast<uint8_t>(rd()); return bytes; }()",
        "params": ["n"]
      },
      "uuid": {
        "template": "[&]{ std::random_device rd; std::mt19937 gen(rd()); std::uniform_int_distribution<> dis(0, 15); auto hex = [](int n) { return \\\"0123456789abcdef\\\"[n]; }; std::string uuid; for (int i = 0; i < 36; ++i) { if (i == 8 || i == 13 || i == 18 || i == 23) uuid += '-'; else if (i == 14) uuid += '4'; else if (i == 19) uuid += hex((dis(gen) & 0x3) | 0x8); else uuid += hex(dis(gen)); } return uuid; }()",
        "params": []
      },
      "selige": {
        "template": "[&]{ std::random_device rd; std::mt19937 gen(rd()); std::uniform_int_distribution<size_t> dis(0, §0.size() - 1); return §0[dis(gen)]; }()",
        "params": ["lista"]
      },
      "miscita": {
        "template": "[&]{ auto v = §0; std::random_device rd; std::mt19937 gen(rd()); std::ranges::shuffle(v, gen); return v; }()",
        "params": ["lista"]
      },
      "semen": {
        "template": "/* seed(§) - C++ requires explicit generator management */",
        "params": ["n"]
      }
    }
    , "innatum": "random"

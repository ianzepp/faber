...
Identifiers and literal contents are encoded as UTF-8 bytes.

Each byte maps directly: `byte → U+2800 + byte`

| Byte/ASCII example | Code | Braille |
|-------------------|------|---------|
| `0`   | 48   | ⠰       |
| `9`   | 57   | ⠹       |
| `A`   | 65   | ⡁       |
...
Keywords are migrating out of the math operator space.

- **Target keyword space:** Geometric Shapes (U+25A0–U+25FF) and related symbol blocks.
- **Current status:** Keyword glyphs listed below are legacy and may change as the migration proceeds.
- **Invariant:** The final mapping must be injective (`token -> glyph` is 1:1 for the canonical token stream).
...
Note: keyword glyphs shown here are legacy. During keyword migration, glyph output will change.
...
```
if (0x2800 <= cp <= 0x28FF) → identifier/literal token (Braille bytes)
if (0x2580 <= cp <= 0x259F) → delimiter token (Block Elements)
if (0x2200 <= cp <= 0x22FF) → operator token (Math Operators)
if (0x2A00 <= cp <= 0x2AFF) → operator token (Supplemental Math Operators)
if (0x25A0 <= cp <= 0x25FF) → keyword token (Geometric Shapes)
else → punctuation or whitespace
```

Note: During migration, some keywords may still temporarily live in the operator range. The long-term goal is strict range separation for unambiguous classification.

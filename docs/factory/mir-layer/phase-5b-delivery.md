...
After Phase 5A, Faber can express a typed alternate exit path in function signatures. Phase 5B should make MIR represent that path explicitly without lowering it to Rust `Result`, exceptions, stack unwinding, or target-specific runtime behavior.

This phase should not invest in `tempta` as the local handling primitive. The next phase will remove `tempta` from the core language surface and make `cape` attach to structured statements such as `fac`, `dum`, and conditional arms. Phase 5B therefore lowers only the function-level alternate exit contract and leaves all local handling forms fail-closed.
...
- Treat `tempta`, `cape`, and `demum` as unsupported legacy/local-handler surfaces during MIR lowering.
- Do not descend into `tempta` bodies to recover inner `iace` statements.
- Keep caller-side failable-call handling and propagation deferred to later phases, starting with local structured handling in Phase 5C.
...
- Phase 5A still has legacy local-handler tolerance around `tempta` / `cape`; Phase 5B must not deepen or depend on that surface because Phase 5C will replace it.
...
- Any diagnostic for `tempta` should describe it as a deferred legacy handler surface, not as the future canonical local handling model.
...
11. Keep `Tempta` fail-closed with an explicit legacy-handler diagnostic.
12. Keep failable direct calls out of MIR lowering until Phase 5C defines local handler edges; Phase 5A should reject unhandled calls before MIR.
...
- `tempta` / `cape` / `demum` remain unsupported with explicit diagnostics and are not hardened as the canonical handler model.
- Source-level failable calls remain rejected before MIR unless and until Phase 5C or a later caller-side phase defines propagation or local handling.
...
Legacy local handling remains unsupported:
...
Canonical local handling is defined in Phase 5C, not Phase 5B:
...
- `tempta` migration or removal.
- `cape` attachment to structured statements.
- Caller-side propagation syntax.
- Caller-side local handling syntax beyond failing closed.
...
- Negative MIR tests for unsupported/deferred `tempta`.
...
Phase 5B is complete when MIR explicitly represents the typed alternate exit path for failable functions, `iace` lowers to `return_error`, `mori` lowers to fatal unreachable flow, local handling remains fail-closed, `tempta` is not further entrenched, and existing target codegen behavior is unchanged.

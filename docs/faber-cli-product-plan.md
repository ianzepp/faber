...
Internal planning artifact for promoting `compilers/radix-rs` from an implementation-named debug CLI into the public-facing Faber compiler command.
...
`radix-rs` is the implementation name, not the language/tool identity. A public compiler should be named after the product, not the current compiler backend.
...
`fabercc` only becomes attractive if the ecosystem is expected to split into multiple low-level compiler executables with a separate top-level `faber` meta-tool. The repo is not there yet. A clean public compiler command should be the shortest obvious name.
...
Promote `radix-rs`, not `nanus-go`, into the public command.
...
- the `radix-rs` binary name as the public contract
...
1. introduce `faber` as the canonical binary
2. keep `radix-rs` for one short transition period if needed
3. print explicit deprecation notices from `radix-rs`
4. remove old compatibility paths after the transition window

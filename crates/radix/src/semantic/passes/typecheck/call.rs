...
impl<'a> TypeChecker<'a> {
    ...
    pub(super) fn build_call_signature(&mut self, args: &mut [HirCallArg]) -> FuncSig {
        let params = args
            .iter_mut()
            .map(|arg| ParamType { ty: self.check_expr(&mut arg.expr), mode: ParamMode::Owned, optional: false })
            .collect();
    ...
    pub(super) fn check_spread_array_compat(&mut self, sig: &FuncSig, arg: &mut HirCallArg) -> bool {
        if !arg.spread {
            return false;
        }
        ...
        for param in &sig.params {
            self.unify(inner, param.ty, arg.span, "argument type mismatch");
        }
        true
    }
    ...
    /// considered.
    pub(super) fn check_call_args(&mut self, sig: &FuncSig, args: &mut [HirCallArg], span: crate::lexer::Span) {
        let required = sig.params.iter().filter(|param| !param.optional).count();
        let spread_compat =
            args.len() == 1 && sig.params.len() > 1 && self.check_spread_array_compat(sig, &mut args[0]);
        ...
        for (arg, param) in args.iter_mut().zip(sig.params.iter()) {
            let arg_ty = self.check_expr_with_expected(&mut arg.expr, Some(param.ty));
    ...
    pub(super) fn check_call(&mut self, callee: &mut HirExpr, args: &mut [HirCallArg]) -> TypeId {

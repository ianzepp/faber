use crate::exempla_e2e::wasm_behavior_fixtures::{
    behavior_matches, expected_wasm_behavior, WASM_BEHAVIOR_FIXTURES,
};
...
fn wasm_behavior_fixtures_match_stub_host_diag_traces() {
    ...
    for fixture in WASM_BEHAVIOR_FIXTURES {
        ...
        let analysis =
            crate::driver::analyze_source(&session, &path.display().to_string(), &source).unwrap();
        let mir = crate::mir::lower_analyzed_unit_with_context(&analysis).unwrap();
        let wat = crate::mir::emit_wasm_text_probe_with_context(
            &mir.program,
            &mir.validation,
            &analysis.interner,
        )
